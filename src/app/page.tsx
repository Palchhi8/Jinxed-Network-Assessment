...
const MODELS = [
  { id: 'stabilityai/stable-diffusion-xl-base-1.0', name: 'SDXL 1.0 High Resolution', desc: 'Full composition depth and high artistic detail' },
  { id: 'stabilityai/sdxl-turbo', name: 'SDXL Turbo Fast Generation', desc: 'Single-step inference for ultra-fast response' },
  { id: 'black-forest-labs/FLUX.1-dev', name: 'FLUX Dev Experimental', desc: 'State-of-the-art composition and detail depth' }
];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 space-y-6">
            <form onSubmit={handleSubmit} className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4 shadow-2xl backdrop-blur-md space-y-4">
              ...
              <div className={`grid transition-[grid-template-rows,opacity] duration-300 ease-in-out ${showAdvanced ? 'grid-rows-[1fr] opacity-100 mt-4' : 'grid-rows-[0fr] opacity-0 pointer-events-none'}`}>
                <div className="overflow-hidden">
                  <div className="border border-zinc-800 bg-zinc-950/40 rounded-2xl p-4 space-y-4 shadow-inner shadow-zinc-950/30">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {/* Model Picker */}
                      <div className="col-span-1 sm:col-span-2 space-y-1.5">
                        <div className="flex items-center justify-between">
                          <label className="text-xs font-semibold text-zinc-300">Generative Model</label>
                          <span className="text-[10px] text-zinc-500">Neural Architecture Selection</span>
                        </div>
                        <select
                          value={model}
                          onChange={(e) => setModel(e.target.value)}
                          className="w-full rounded-xl border border-zinc-800 bg-zinc-950 px-3.5 py-2 text-xs text-zinc-300 outline-none focus:border-violet-500/50 hover:border-zinc-700 hover:bg-zinc-900/40 cursor-pointer transition-all duration-200"
                        >
                          {MODELS.map(m => (
                        ...
                        <p className="text-[10px] text-zinc-500 leading-tight">
                          {MODELS.find(m => m.id === model)?.desc || "Select a specialized deep-learning neural architecture"}
                        </p>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        <div>
                          ...
                          <p className="text-[10px] text-zinc-500 leading-tight">Choose the composition format for the generated image</p>
                        </div>
                          ...
                          {['1:1', '16:9', '9:16'].map(ratio => (
                            <button
                              ...
                              className={`py-1.5 text-[10px] font-bold rounded-lg transition-all cursor-pointer ${
                                aspectRatio === ratio
                                  ? 'bg-zinc-900 text-violet-300 border border-violet-500/20 shadow-md shadow-violet-950/20'
                      ...
                      <div className="space-y-1.5">
                        <div className="flex justify-between items-start">
                          <div>
                            <label className="text-xs font-semibold text-zinc-300">Prompt Adherence</label>
                            <p className="text-[10px] text-zinc-500 leading-tight font-normal">Controls how closely the AI follows your prompt</p>
                          </div>
                          <span className="text-[10px] text-zinc-400 font-mono font-bold px-1.5 py-0.5 rounded bg-zinc-900 border border-zinc-800">{guidanceScale}</span>
                        </div>
                        <div className="pt-1">
                          <input
                            type="range"
                            ...
                            className="w-full accent-violet-600 hover:accent-violet-500 h-1.5 rounded-lg bg-zinc-900 cursor-pointer transition-all duration-200"
                          />
                        </div>
                      </div>

                      {/* Inference Steps (Quality Steps) */}
                      <div className="col-span-1 sm:col-span-2 space-y-1.5 border-t border-zinc-900 pt-3">
                        <div className="flex justify-between items-start">
                          <div>
                            <label className="text-xs font-semibold text-zinc-300">Quality Steps</label>
                            <p className="text-[10px] text-zinc-500 leading-tight">Higher values improve image detail but increase generation time</p>
                          </div>
                          <span className="text-[10px] text-zinc-400 font-mono font-bold px-1.5 py-0.5 rounded bg-zinc-900 border border-zinc-800">{steps} steps</span>
                        </div>
                        <div className="pt-1">
                          <input
                            type="range"
